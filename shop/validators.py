...
def height_validator(category, height):
    if category == "FW":
        if height >= alu_info.FIX_MIN_HEIGHT and height <= alu_info.FIX_MAX_HEIGHT:
            ...
            raise ValidationError(f"Height of the FIX windows should be between {alu_info.FIX_MIN_HEIGHT} and {alu_info.FIX_MAX_HEIGHT} ")
    elif category == 'CW':
        if height >= alu_info.CASEMENT_MIN_HEIGHT and height <= alu_info.CASEMENT_MAX_HEIGHT:
            return height
        else: 
            raise ValidationError(f"Height of the CASEMENT windows should be between {alu_info.CASEMENT_MIN_HEIGHT} and {alu_info.CASEMENT_MAX_HEIGHT} ")
    else:
        if height >= alu_info.SLIDE_MIN_HEIGHT and height <= alu_info.SLIDE_MAX_HEIGHT:
            return height
        else: 
            raise ValidationError(f"Height of the SLIDING windows should be between {alu_info.SLIDE_MIN_HEIGHT} and {alu_info.SLIDE_MAX_HEIGHT} ")
